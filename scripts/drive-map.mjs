...
async function main() {
  ...
  const drawn = (page) =>
    page.evaluate(() => {
      ...
      return {
        ...
        // it passes (docs/decisions/0028-the-centre-is-named.md), so the doorways can be out of
        // sight with the centre they belong to — which is what makes the warning below a
        // judgement about the picture rather than about one element.
  ...
  // The centre is named and a pan is looking: docs/decisions/0028-the-centre-is-named.md. Both
  // readouts, because handing the mark over was never only a label change — every node the
  // middle crossed had its ring read and seated for good, and a seat is permanent.
  //
